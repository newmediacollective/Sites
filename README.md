...
1. Posting new images in done via a simple Flask app served by a Gunicorn daemon (by an http post request to https://{host}/posts)
    a. Images should be uploaded as a common type (png, jpg, etc.)
    b. Images are then converted to progressive jpgs using [ImageMagick](https://imagemagick.org/index.php) and Google's recommended settings for image optimization
