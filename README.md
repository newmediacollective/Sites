# Sites
A set of tools to manage photo stream sites - in a few minutes you can have your own https site and the ability to post photos from anywhere.
...
### Notes
1. The site is served as static content from nginx (out of the `/home/webhost/Sites/app/.sites/{host}` directory)
1. Posts are stored in `app/.sites/data/posts.json`, and the html pages are generated by the `app/content_manager.py` script
1. Posting new images in done via a simple Flask app served by a Gunicorn daemon (by an http post request to https://{host}/posts)
  1. Images should be uploaded as a common type (png, jpg, etc.)
  1. Images are then converted to progressive jpgs using [ImageMagick](https://imagemagick.org/index.php) and Google's recommended settings for image optimization
1. All http traffic is redirected to https
1. All `www.` urls are redirected to non-www, cause that's nicer
1. All trailing slash urls are rewritten to non-trailing slash urls, cause that's nicer
...
Under the hood, it's just an http post request to `https://{host}.posts` (meaning you can use that token to post images from anywhere, like an iOS shortcut):
```
...
### Android
Buy a [real phone](https://www.apple.com/iphone/)! ;) You can also make an app that posts a photo.
